...
class GetInvoice:
    ...
    def save_invoice(customer_data, goods_details, total_amount):
        ...
        for goods in goods_details:
            if goods['description'] == "RICH PALM GOLD 15 KG TIN":
            ...
            elif goods['description'] == "RICH PALM GOLD 14.5 KG TIN":
                RPG14_5KG += int(goods['quantity'])
                total_no_tins += int(goods['quantity'])
                total_quantity += RPG14_5KG * 14.5
                ...
                total_no_tins += int(goods['quantity'])
        try:
            print(customer_data['invoice_no'])
            ...
            Stocks.update_available_tins(tin_counts, "deduct")
    ...
    def monthly_report(month):
        ...
        for invoice in this_month_invoices:
            ...
            for desc in invoice.goods_details:
                ...
                elif desc['description'] == "RICH PALM GOLD 14.5 KG TIN":
                    RPG14_5KG += int(desc['quantity'])
        ...
        tins_desc = ['RPG15KG', 'RPG15LTR', 'RPG13KG', 'RPG14.5KG', 'HG15KG', 'HG15LTR']
        no_of_tins = [RPG15KG, RPG15LTR, RPG13KG, RPG14_5KG, HG15KG, HG15LTR]
        total_tins = RPG15KG + RPG15LTR + RPG13KG + RPG14_5KG + HG15KG + HG15LTR
        months = [month]
        totals = [total_amount]
        ...
        return chart_html, cust_list, totals, months, total_oil, this_month_invoices
...
class Stocks:
    ...
    def get_stock():
        ...
        if not latest:
            ...
            latest = db.session.query(Stock).order_by(Stock.date.desc()).first()
            return latest
        elif latest.date == datetime.today().date():
            return latest
        else:
            new_stock = Stock(
                oil={
                ...
                other={
                    "15KG": latest.other['15KG'],
                    "15LTR": latest.other['15LTR'],
                    "14_5KG": latest.other['14_5KG'],
                    "13KG": latest.other['13KG']
                },
                date=datetime.today().date()
            )
            db.session.add(new_stock)
    ...
    @staticmethod
    def update_available_tins(available_tins, method):
        result = db.session.query(Stock).order_by(Stock.date.desc()).first()
        update_tins = result.other
        print(update_tins)
        print(available_tins)
        if method == "add":
            update_tins = {
                "15KG": int(update_tins['15KG']) + int(available_tins['15KG']),
                "15LTR": int(update_tins['15LTR']) + int(available_tins['15LTR']),
                "14_5KG": int(update_tins['14_5KG']) + int(available_tins['14_5KG']),
                "13KG": int(update_tins['13KG']) + int(available_tins['13KG']),
            }
            result.other = update_tins
            db.session.commit()
        elif method == "deduct":
            update_tins = {
                "15KG": int(update_tins['15KG']) - int(available_tins['15KG']),
    ...
    def get_all_daily_stocks():
        ...
        for today in result:
            ...
            KG14_5 = int(today.tins['RPG14_5KG'])
            KG13 += int(today.tins['RPG13KG'])
            total_tins += int(today.tins['RPG15KG']) + int(today.tins['RPG15LTR']) + int(today.tins['RPG13KG']) + int(today.tins['RPG14_5KG'])
